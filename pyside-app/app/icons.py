...
ICONS_DIR = Path(os.environ.get('APPDATA', '')) / 'Orbit' / 'icons'
ICONS_DIR.mkdir(parents=True, exist_ok=True)


def icon_cache_path(url: str) -> Path:
    h = hashlib.md5(url.encode()).hexdigest()
    return ICONS_DIR / f'{h}.png'


def get_cached_pixmap(url: str) -> Optional[QPixmap]:
...
class IconFetcher(QObject):
    fetched = Signal(str, QPixmap)  # url, pixmap
    ...
    def _on_finished(self, reply: QNetworkReply):
        ...
        if reply.error() == QNetworkReply.NetworkError.NoError:
            ...
            if not px.isNull():
                px = px.scaled(32, 32)
                px.save(str(icon_cache_path(url)))
                self.fetched.emit(url, px)
        reply.deleteLater()


# ── SVG icon system (Heroicons outline, 24×24 viewBox) ───────────────────────
...
_PATHS: dict[str, str] = {
    ...
    "bell-slash": '<path stroke-linecap="round" stroke-linejoin="round" d="M9.143 17.082a24.248 24.248 0 0 0 3.844.148m-3.844-.148a23.856 23.856 0 0 1-5.455-1.31 8.964 8.964 0 0 0 2.3-5.542m3.155 6.852a3 3 0 0 0 5.667 1.97m1.965-2.277L21 21m-4.225-4.225a23.81 23.81 0 0 0 .56-2.072m0 0a23.909 23.909 0 0 0 .418-1.748 8.926 8.926 0 0 0-.56-3.032m0 0a23.876 23.876 0 0 0-.56 2.072M6.228 6.228 3 3m3.228 3.228 3.65 3.65m7.894 7.894L21 21m-3.228-3.228-3.65-3.65m0 0a3 3 0 1 0-4.243-4.243m4.242 4.242L6.227 6.227"/>',
    ...
    "signal": '<path stroke-linecap="round" stroke-linejoin="round" d="M9.348 14.652a3.75 3.75 0 0 1 0-5.304m5.304 0a3.75 3.75 0 0 1 0 5.304m-7.425 2.121a6.75 6.75 0 0 1 0-9.546m9.546 0a6.75 6.75 0 0 1 0 9.546M5.106 18.394a10.5 10.5 0 0 1 0-12.788m13.788 0a10.5 10.5 0 0 1 0 12.788M12 12h.008v.008H12V12Z"/>',
...
def _make_svg(path_data: str, color: str, size: int) -> bytes:
    svg = (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" '
        f'fill="none" viewBox="0 0 24 24" stroke="{color}" stroke-width="1.5">'
        f'{path_data}'
        f'</svg>'
    )
    return svg.encode()
...
def icon(name: str, size: int = 16, color: str = '#c6d0f5') -> QIcon:
    """Return a QIcon for the given Heroicon name, size, and color."""
    path_data = _PATHS.get(name, _PATHS['information-circle'])
    svg_bytes = _make_svg(path_data, color, size)
